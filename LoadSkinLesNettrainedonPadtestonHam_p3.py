...
def main():
    print("🚀 Starting SkinLesNet testing (trained on PAD, testing on HAM) with final metrics...")
    ...
    results_dir = os.path.join(base_results_dir, "LoadSkinLesNettrainedonPadtestonHam_p3")
    
    os.makedirs(results_dir, exist_ok=True) # Ensure the final results directory exists
    
    results_txt_path = os.path.join(results_dir, "result_skinlesnet_pad_test_ham.txt")
    conf_matrix_path = os.path.join(results_dir, "confusion_matrix_skinlesnet_pad_test_ham.png")
    ...
    # METRICS CLASSIFICATION: True labels 0-6 (HAM), predictions 0-5 (PAD).
    # Since the prediction range (0-5) is less than the true label range (0-6), 
    # the classification report will automatically treat true label 6 as unclassified or misclassified.
    ...
    with open(results_txt_path, "w") as f:
        f.write("--- SkinLesNet (Trained on PAD) Test Results on HAM ---\n\n")
