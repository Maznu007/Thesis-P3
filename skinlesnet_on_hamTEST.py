...
def main():
    print("🚀 Evaluating PAD-trained SkinLesNet on HAM dataset...")
    ...
    save_dir       = r"D:\dataset\ham\organized_ham\skinlesnet_from_PAD_result"
    ...
    # Load HAM dataset
    # ===============================
    ham_dataset = ImageFolder(ham_test_path, transform=transform)
    ham_loader  = DataLoader(ham_dataset, batch_size=32, shuffle=False)
    ...
    pad_classes = ["actinic keratosis", "basal cell carcinoma", "melanoma", "nevus",
                   "seborrheic keratosis", "squamous cell carcinoma"]

    # Build HAM → PAD index mapping
    ham_to_pad = {ham_classes.index(c): pad_classes.index(c) for c in overlap_classes}
    print("🔄 HAM → PAD index mapping:", ham_to_pad)
    ...
    with torch.no_grad():
        for imgs, labels in ham_loader:
            imgs = imgs.to(device)
            outputs = model(imgs)
            preds = torch.argmax(outputs, dim=1).cpu().numpy()

            for t, p in zip(labels.numpy(), preds):
                if t in ham_to_pad:  # only keep overlap classes
                    all_labels.append(ham_to_pad[t])  # true mapped to PAD idx
                    all_preds.append(p)
    ...
    sns.heatmap(cm, annot=True, fmt='d',
                ...
                cmap="Blues")
    plt.title("Confusion Matrix (PAD-trained SkinLesNet on HAM Overlap)")
    ...
    with open(results_txt_path, "w", encoding="utf-8") as f:
        f.write("=== Evaluation of PAD-trained SkinLesNet on HAM (Overlap Classes) ===\n\n")
