...
def main():
    print("🚀 Starting PAD (6-class) evaluation with HAM-trained RegNetY-32GF...")
    ...
    save_dir = os.path.join(base_dir, "regnety320_on_PAD_result")
    os.makedirs(save_dir, exist_ok=True)

    model_save_path = os.path.join(save_dir, "regnety320_on_PAD.pth")
    results_txt_path = os.path.join(save_dir, "results_PAD.txt")
    pred_csv_path = os.path.join(save_dir, "predictions_PAD.csv")
    conf_matrix_path = os.path.join(save_dir, "confusion_matrix_PAD.png")
    ...
    # Metrics only for overlapping classes
    print("📊 Calculating metrics for overlapping classes...")
    ...
    for t, p in zip(all_labels, all_preds):
        if t in pad_to_ham:
            filtered_labels.append(pad_to_ham[t])
    ...
    with open(results_txt_path, "w", encoding="utf-8") as f:
        f.write("=== Evaluation of HAM-trained RegNetY-32GF on PAD 6-class dataset ===\n\n")
