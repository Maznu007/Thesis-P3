...
def main():
    print("🚀 Starting SkinLesNet testing (trained on HAM, testing on PAD) with final metrics...")
    ...
    results_dir = os.path.join(base_results_dir, "LoadSkinLesNettrainedonHAMtestonPad_p3")
    
    os.makedirs(results_dir, exist_ok=True) # Ensure the final results directory exists
    
    results_txt_path = os.path.join(results_dir, "result_skinlesnet_ham_test_pad.txt")
    conf_matrix_path = os.path.join(results_dir, "confusion_matrix_skinlesnet_ham_test_pad.png")
    ...
    # METRICS CLASSIFICATION: True labels 0-5 (PAD), predictions 0-6 (HAM).
    # Clamping predictions (0-6) to the max target index (0-5) for metric consistency.
    ...
    with open(results_txt_path, "w") as f:
        f.write("--- SkinLesNet (Trained on HAM) Test Results on PAD ---\n\n")
