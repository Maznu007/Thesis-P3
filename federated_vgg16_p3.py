...
from torchvision.models import vgg16, VGG16_Weights # Import VGG16
...
# ==============================================================================
# 1. MODEL AND LOSS DEFINITIONS (VGG16 + FEDERATED SPLIT)
# ==============================================================================
...
class ClassificationHead(nn.Module):
    def __init__(self, in_features, num_classes):
        ...
        # VGG16 outputs a feature vector of 4096 from the last layer of its classifier block
...
# Feature Extractor (shared across clients, to be averaged)
class VGG16FeatureExtractor(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # The feature extractor part is the 'features' block (convolutional layers)
        # and the first two layers of the 'classifier' block in standard VGG16.
        # We take the entire backbone and remove the final classification layer.
        
        # Standard VGG16 classifier is sequential: (0) Linear, (3) Linear, (6) Linear(4096, 1000)
        # We want the output before the final 1000-class layer.
        
        # Extract all layers except the last Linear layer in the classifier
        self.backbone.classifier = self.backbone.classifier[:-1]
        
        # Add an Identity layer to correctly output the feature vector (4096)
        # VGG16's feature vector size after the second-to-last linear layer is 4096
        self.in_features = 4096 

    def forward(self, x):
        # The 'features' block (conv layers) and the first 2 linear layers run sequentially
        features = self.backbone(x)  # shape: (B, 4096)
        return features

# Full client model (feature extractor + client-specific classifier head)
class VGG16Client(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(VGG16Client, self).__init__()
        self.feature_extractor = VGG16FeatureExtractor(pretrained=pretrained)
...
# ==============================================================================
# 2. HELPER FUNCTIONS: DATA LOADING AND FL OPERATIONS (Identical to previous code)
# ==============================================================================
...
def fed_avg(client_weights, client_sizes):
    ...
    # Ensure all tensors are float and zero-initialized for summation
    ...
    for client_id, weights in client_weights.items():
        ...
        for key in global_weights.keys():
            global_weights[key].add_(weights[key].float(), alpha=weight_factor)

    return global_weights


# ==============================================================================
# 3. TRAINING AND EVALUATION LOGIC (Identical to previous code)
# ==============================================================================
...
# ==============================================================================
# 4. MAIN FEDERATED LEARNING EXECUTION WITH VGG16
# ==============================================================================

def run_federated_learning_vgg16():
    """Main function to run the FedAvg simulation with VGG16."""
    print("🚀 Setting up Federated Learning with VGG16...")
    ...
    LOCAL_EPOCHS = 4
    BATCH_SIZE = 32
    OUTPUT_FILE = "federated_vgg16_results.txt"
    ...
    # ImageNet mean and std for VGG16
    ...
    for client_id, paths in client_paths.items():
        ...
        # Initialize client model (using VGG16Client)
        model = VGG16Client(num_classes=num_classes, pretrained=True).to(DEVICE)
    ...
    # Initialize global weights from one client
    ...
    print(f"\n🧠 Starting FedAvg Training for {GLOBAL_EPOCHS} Global Epochs (VGG16)...")

    for g_epoch in range(1, GLOBAL_EPOCHS + 1):
        print(f"\n======== Global Epoch {g_epoch}/{GLOBAL_EPOCHS} ========")
        client_updates = {}

        # 1. Distribute global model + local training
        for client_id, model in client_models.items():
            ...
            client_shared_weights, local_loss, local_acc = train_client(
                ...
                criterion=FocalLoss(alpha=1, gamma=2).to(DEVICE), # Using Focal Loss
    ...
    # --- Final Global Model Evaluation on Client Test Sets ---
    print("\n\n===========================================")
    print("🔍 Final Global Model Evaluation on Test Sets (VGG16)")
    print("===========================================")
    ...
    # --- Save Results to Text + Confusion Matrices ---
    print(f"\n\n💾 Saving all results to {OUTPUT_FILE}...")

    with open(OUTPUT_FILE, "w") as f:
        f.write("=" * 80 + "\n")
        f.write(" FEDERATED LEARNING RESULTS: VGG16 (FedAvg) \n")
        f.write("=" * 80 + "\n")
        f.write("Parameters:\n")
        f.write(f"  Global Epochs: {GLOBAL_EPOCHS}\n")
        f.write(f"  Local Epochs: {LOCAL_EPOCHS}\n")
        f.write(f"  Clients: {', '.join(client_data.keys()).upper()}\n")
        f.write("  Aggregation: Weighted Averaging (by sample size)\n")
        f.write("  Averaged Layers: Feature Extractor (VGG16 backbone)\n")
        f.write("-" * 80 + "\n\n")
        ...
        for g_epoch in range(GLOBAL_EPOCHS):
            ...
            for client_id in client_data.keys():
                line += (
                    f" {fl_history[client_id]['local_loss'][g_epoch]:<10.4f} | "
                    f"{fl_history[client_id]['local_acc'][g_epoch]:<9.4f} |"
                )
            f.write(line + "\n")
        ...
        for client_id, metrics in final_metrics.items():
            ...
            try:
                ...
                plt.tight_layout()
                conf_matrix_path = f"conf_matrix_vgg16_{client_id}_final.png"
    ...
    print("\n✅ Federated VGG16 training complete!")
