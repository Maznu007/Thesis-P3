...
def main():
    print("🚀 Starting SkinLesNet testing (trained on PAD, testing on ISIC) with final metrics...")

    # --- Paths and Constants ---
    print("📁 Setting paths and constants...")
    
    # Model Loading Constants
    PAD_NUM_CLASSES = 6  # The model was trained and saved with 6 output classes
    ...
    # CORRECT MODEL LOADING PATH (SkinLesNet trained on PAD, using the path structure provided by the user)
    model_load_path = os.path.join(pad_base_dir, "result_skinlesnet", "skinlesnet_model.pth")
    ...
    results_dir = os.path.join(base_results_dir, "LoadSkinLesNettrainedonPadtestonISIC_p3")
    
    os.makedirs(results_dir, exist_ok=True) # Ensure the final results directory exists
    
    results_txt_path = os.path.join(results_dir, "result_skinlesnet_pad_test_isic.txt")
    conf_matrix_path = os.path.join(results_dir, "confusion_matrix_skinlesnet_pad_test_isic.png")
    ...
    print("🧠 Loading SkinLesNet model architecture (6 classes for weight compatibility)...")
    # Initialize the model with the class count it was TRAINED with (6)
    model = SkinLesNet(num_classes=PAD_NUM_CLASSES)
    ...
    # METRICS CLASSIFICATION: True labels 0-8 (ISIC), predictions 0-5 (PAD).
    # Since the prediction range (0-5) is less than the true label range (0-8),
    # the classification report will automatically treat true labels 6, 7, and 8 
    # as unclassified or misclassified.
    ...
    with open(results_txt_path, "w") as f:
        f.write("--- SkinLesNet (Trained on PAD) Test Results on ISIC ---\n\n")
