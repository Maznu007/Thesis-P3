...
# ==============================================================================
# 1. MODEL AND LOSS DEFINITIONS (REGNETY-32GF + FEDERATED SPLIT)
# ==============================================================================
...
class RegNetFeatureExtractor(nn.Module):
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            self.backbone = regnet_y_32gf(weights='DEFAULT')
        else:
            self.backbone = regnet_y_32gf(weights=None)

        # RegNet final classifier is `fc`
...
class RegNetClient(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(RegNetClient, self).__init__()
...
def fed_avg(client_weights, client_sizes):
    ...
    Ensures tensors are float to avoid type mismatch with BN stats.
    """
...
# ==============================================================================
# 4. MAIN FEDERATED LEARNING EXECUTION WITH REGNETY-32GF
# ==============================================================================

def run_federated_learning_regnet():
    """Main function to run the FedAvg simulation with RegNetY-32GF."""
    print("🚀 Setting up Federated Learning with RegNetY-32GF...")
    ...
    LOCAL_EPOCHS = 4
    BATCH_SIZE = 32
    OUTPUT_FILE = "federated_regnet_results.txt"
    ...
    print(f"\n🧠 Starting FedAvg Training for {GLOBAL_EPOCHS} Global Epochs (RegNetY-32GF)...")
    ...
    print("\n\n===========================================")
    print("🔍 Final Global Model Evaluation on Test Sets (RegNetY-32GF)")
    print("===========================================")
    ...
    with open(OUTPUT_FILE, "w") as f:
        f.write("=" * 80 + "\n")
        f.write(" FEDERATED LEARNING RESULTS: RegNetY-32GF (FedAvg) \n")
        f.write("=" * 80 + "\n")
        f.write("Parameters:\n")
        ...
        f.write("  Averaged Layers: Feature Extractor (RegNetY-32GF backbone)\n")
        f.write("-" * 80 + "\n\n")
        ...
        for client_id, metrics in final_metrics.items():
            ...
            try:
                ...
                plt.tight_layout()
                conf_matrix_path = f"conf_matrix_regnet_{client_id}_final.png"
    ...
    print("\n✅ Federated RegNet training complete!")
