...
def main():
    print("🚀 Starting VGG16 testing (trained on HAM, testing on ISIC) with final metrics...")
    ...
    # Model loading path based on user input
    ...
    results_dir = os.path.join(base_results_dir, "LoadVgg16trainedonHAMtestonISIC_p3")
    
    os.makedirs(results_dir, exist_ok=True) # Ensure the final results directory exists
    
    results_txt_path = os.path.join(results_dir, "result_vgg16_ham_test_isic.txt")
    conf_matrix_path = os.path.join(results_dir, "confusion_matrix_vgg16_ham_test_isic.png")
    ...
    # The true labels are 0-8 (ISIC), but predictions are 0-6 (HAM).
    # We must treat the predictions as categories 0-6 against true classes 0-8.
    # To compute P/R/F1, we must clamp the predictions to the true label range (0-8)
    # *if* the predictions exceeded it. Since 7 < 9, clamping is not necessary here
    # to fit the metrics, but the predictions 7 and 8 are impossible.
    
    # We use the true ISIC labels (0-8) as the basis for the classification report.
    ...
    with open(results_txt_path, "w") as f:
        f.write("--- VGG16 (Trained on HAM) Test Results on ISIC ---\n\n")
