...
def main():
    print("🚀 Starting SkinLesNet testing (trained on ISIC, testing on HAM) with final metrics...")
    ...
    results_dir = os.path.join(base_results_dir, "LoadSkinLesNettrainedonISICtestonHam_p3")
    
    os.makedirs(results_dir, exist_ok=True) # Ensure the final results directory exists
    
    results_txt_path = os.path.join(results_dir, "result_skinlesnet_isic_test_ham.txt")
    conf_matrix_path = os.path.join(results_dir, "confusion_matrix_skinlesnet_isic_test_ham.png")
    ...
    # METRICS CLASSIFICATION: True labels 0-6 (HAM), predictions 0-8 (ISIC).
    # Clamping predictions (0-8) to the max target index (0-6) for metric consistency.
    ...
    with open(results_txt_path, "w") as f:
        f.write("--- SkinLesNet (Trained on ISIC) Test Results on HAM ---\n\n")
