"""
Federated RegNet with SE-Attention Fixed Dropout & Mask-Aware Aggregation
============================================================================

Same strategy as your pasted "Federated DermaNet..." code:
- Fixed dropout masks ONLY in the SE-Attention block (safe for pretrained backbone)
- Mask-aware FedAvg aggregation (avoids zero dilution)
- Client-specific attention specialization for non-IID skin cancer data

ONLY CHANGE:
- Backbone changed from EfficientNet-B3 (DermaNet) to RegNetY-32GF (your RegNet structure)
"""
...
OUTPUT_FILE = "regnet_se_fixed_dropout_results.txt"


# ==============================================================================
# 1. SE-ATTENTION BLOCK WITH FIXED DROPOUT (same as your DermaNet demo)
# ==============================================================================
...
# ==============================================================================
# 2. FEATURE EXTRACTOR WITH REGNET BACKBONE + EXPLICIT SE-ATTENTION
# ==============================================================================

class RegNetFeatureExtractor(nn.Module):
    """
    Feature extractor with pretrained RegNet backbone + SE-Attention.

    Key Design:
    - Backbone: Preserved with pretrained weights (NOT touched by dropout)
    - SE-Attention: Added AFTER the backbone with fixed client-specific dropout
    - This matches your DermaNet demo logic exactly; only backbone changes.
    """

    def __init__(self, pretrained=True, se_dropout_rate=0.3, client_seed=None):
        super(RegNetFeatureExtractor, self).__init__()

        # Load pretrained RegNetY-32GF backbone (PRESERVED)
        # Your code uses: regnet_y_32gf(weights='DEFAULT')
        # We'll try the modern weights enum if available; otherwise fallback.
        try:
            ...
            self.backbone = regnet_y_32gf(weights='DEFAULT' if pretrained else None)

        # RegNet classification head is .fc (like your code)
        self.in_features = self.backbone.fc.in_features

        # Remove original classifier (fc)
        self.backbone.fc = nn.Identity()

        # Add SE-Attention with fixed dropout (same strategy)
    ...
    def forward(self, x):
        ...
        features = self.backbone(x)  # shape: [B, C]
...
class RegNetAttentionClient(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=True, se_dropout_rate=0.3, client_seed=None):
        super(RegNetAttentionClient, self).__init__()
        self.feature_extractor = RegNetFeatureExtractor(
...
# ==============================================================================
# 6. MASK-AWARE FEDERATED AVERAGING (same logic as your DermaNet demo)
# ==============================================================================

def mask_aware_fed_avg(client_weights, client_sizes, client_masks):
    """
    Mask-Aware Federated Averaging
    ...
    for param_name in client_weights[first_client].keys():
        # Identify SE-Attention parameters (explicit SE block after RegNet backbone)
...
def run_federated_learning():
    ...
    print(f"\n{'='*80}")
    print("🚀 FEDERATED LEARNING: REGNET + SE-ATTENTION FIXED DROPOUT")
    print("   Strategy from Demo_Idea.pdf (same as your DermaNet demo, backbone swapped)")
    print(f"{'='*80}\n")
    ...
    # Data transforms (same as your demo)
    ...
    for idx, (client_id, paths) in enumerate(CLIENT_DATASETS.items()):
        ...
        # Deterministic seed for this client
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write("="*80 + "\n")
        f.write(" FEDERATED LEARNING RESULTS: REGNET + SE-ATTENTION FIXED DROPOUT\n")
        ...
        for client_id, metrics in final_metrics.items():
            ...
            try:
                ...
                cm_file = f"confusion_matrix_{client_id}_regnet_se_dropout.png"
